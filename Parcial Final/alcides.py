...
sig = np.where(sig > (np.mean(sig) + 1*np.std(sig)), np.mean(sig), sig)
sig = np.where(sig < (np.mean(sig) - 1*np.std(sig)), np.mean(sig), sig)

sig = np.abs(sig)
b,a = signal.butter(5, 3/(fs/2), btype='lowpass')
...
for i in range(0,np.size(l_izq)-1):
    ...
    while (l_izq[0][i] > l_der[0][ind_derecho]):
        ind_derecho = ind_derecho + 1
    ncontracciones = ncontracciones + 1
